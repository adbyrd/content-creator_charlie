...
$w.onReady(function () {
    console.log('[cc-v2.0.0] Profile Settings Page Initialized');
    bootUI();
});
...
async function openSettingsModal(modalId) {
    ...
    try {
        console.log(`[cc-v2.0.0] Opening modal: ${modalId}`);
        ...
        console.error(`[cc-v2.0.0] Error opening modal ${modalId}:`, err);
    }
}
...
function handleUpdateSuccess() {
    ...
    console.log('[cc-v2.0.0] Profile updated. Triggering page reload.');

    setTimeout(() => {
        wixWindow.postMessage("reload_page");
        wixWindow.location.to(wixWindow.location.url); 
    }, 1500);
}
