...
$w.onReady(async function () {
    console.log('[cc-v1.0.4] Category Modal Initialized');
    await bootModal();
});
...
async function bootModal() {
    ...
    try {
        ...
        if (res && res.ok) {
            _taxonomyCache = res;
            setupDropdowns();
            if (_profileContext) hydrateForm(_profileContext);
        } else {
            console.error("[cc-v1.0.4] Taxonomy failed. Check if 'active' field is set to true in DB.", res);
        }
    } catch (err) {
        console.error('[cc-v1.0.4] Modal Boot Error:', err);
    }
}
...
async function handleSave() {
    ...
    try {
        ...
        console.error(`[cc-v1.0.4] Save error:`, err);
