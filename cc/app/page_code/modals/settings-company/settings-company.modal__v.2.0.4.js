...
$w.onReady(function () {
    console.log('[cc-v2.0.4] Company Settings Modal Initialized');
    bootModal();
});
...
async function bootModal() {
    ...
    if (context && context.profile) {
        console.log('[cc-v2.0.4] Hydrating form with existing profile data');
...
async function handleSave() {
    ...
    if (!validation.isValid) {
        showModalError(validation.message, $w); // Pass $w so utility sees modal elements
        return;
    }
    ...
    try {
        ...
        console.error(`[cc-v2.0.4] Save failed:`, err);
        importMasterPageErrorHandler(err.message);
    } finally {
        toggleLoading(false);
    }
}
...
function importMasterPageErrorHandler(message) {
    wixWindow.postMessage({ type: "SHOW_MODAL_ERROR", message });
}