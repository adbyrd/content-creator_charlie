...
$w.onReady(function () {
    console.log('[cc-v2.0.3] Company Settings Modal Initialized');
    bootModal();
});
...
async function bootModal() {
    ...
    if (context && context.profile) {
        console.log('[cc-v2.0.3] Hydrating form with existing profile data');
...
async function handleSave() {
    ...
    if (!validation.isValid) {
        importMasterPageErrorHandler(validation.message);
        return;
    }
    ...
    try {
        ...
        console.error(`[cc-v2.0.3] Save failed:`, err);
