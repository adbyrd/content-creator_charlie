 ...
 * Version: [Project Service: v1.3.1]
 */
...
async function getAuthenticatedMemberId() {
    try {
        ...
        console.error('[Project Service: v1.3.1] Identity check failed:', err);
        return null;
    }
}
...
export const createProject = webMethod(Permissions.Anyone, async (projectData) => {
    try {
        ...
        console.log(`[Project Service: v1.3.1] Creating project for owner: ${memberId}`);
        ...
        const payload = {
            ...
            owner: memberId // Explicitly set to ensure multi-tenancy isolation
        };
        ...
        console.error(`[Project Service: v1.3.1] createProject Error:`, err);
...
export const getUserProjectCount = webMethod(Permissions.Anyone, async () => {
    try {
        ...
        console.error(`[Project Service: v1.3.1] getUserProjectCount Error:`, err);
...
export const getMyProjects = webMethod(Permissions.Anyone, async () => {
    try {
        ...
        console.error(`[Project Service: v1.3.1] getMyProjects Error:`, err);
        return { ok: false, error: err.message };
    }
});