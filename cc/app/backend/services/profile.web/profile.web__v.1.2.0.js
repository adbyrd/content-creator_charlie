...
export const checkMemberExists = webMethod(Permissions.Anyone, async (email) => {
    ...
    const logPrefix = `${MODULE_ID} [req:${requestId}]`;

    // 1. Defensive Guard: Backend Payload Validation
    if (!email || typeof email !== 'string' || !email.includes('@')) {
        console.warn(`${logPrefix} Security Alert: Invalid payload rejected.`);
        return { 
            ok: false, 
            exists: false, 
            error: { type: 'VALIDATION_ERROR', message: 'Invalid identity format' } 
        };
    }
    ...
    try {
        console.log(`${logPrefix} Initiating identity lookup for: ${sanitizedEmail}`);

        // 2. Resilience: Query with Exponential Backoff
        const query = wixData.query(MEMBERS_COLLECTION)
            .eq("loginEmail", sanitizedEmail)
            .limit(1);

        const results = await queryWithRetry(query);
        const exists = results.items.length > 0;

        console.log(`${logPrefix} Lookup complete. Identity Found: ${exists}`);

        // 3. Contract-Based Response
        return {
            ok: true,
            exists: exists,
        ...
        // 4. Observability: Structured Error Logging
        console.error(`${logPrefix} Critical Database Failure:`, {
            message: err.message,
            stack: err.stack,
        ...
        return {
            ...
            exists: false,
            error: { 
                type: 'SYSTEM_RESILIENCE_FAILURE', 
                message: 'Identity service currently unavailable.' 
            }
        };
    }
});
